...
int main(void)
{
	...
	char text_top_row[40] = "v :) : :D \0";
	char text_bottom_row[40] = "  :P -_- \0";
...
{
	// Pointer to LCD Address
  	volatile char * LCD_display_ptr = (char *) 0x10003050;	

	//ASCII Characters
	*(LCD_display_ptr + 1) = 0xF6; // ascii address
	*(LCD_display_ptr + 1) = 'h'; // ascii character
	
	// Custom Character 
	char * cgram = 0x00; 	// pointer to memory address 0
	*cgram = {0x05,0x04,0x04,0x02,0x04,0x1A,0x04,0x00}; // character patern
	*(LCD_display_ptr + 1) = *cgram; // set the data from CGRAM into the DDRAM
